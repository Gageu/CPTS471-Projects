...
use utils::{
    parse_args, parse_sequences_from_fasta, print_alignment_summary, print_sequences,
    read_score_config,
};
/*
   -------- Project Structure ---------
...
fn project2_suffix_trees(fasta_file: &str, alphabet_file: &str) {
    ...
    utils::print_tree_summary(&tree, sequences[0].len(), construction_time);

    // Generate comprehensive report
    match File::create(&report_path) {
        Ok(mut file) => {
            if let Err(e) = utils::generate_report(
                &mut file,
                fasta_file,
                &tree,
                &sequences[0],
    ...
    match File::create(&bwt_path) {
        Ok(mut file) => {
            if let Err(e) = suffix_tree::bwt_to_file(&tree, &sequences[0], &mut file) {
